                                ...
                                int num_buffers)
{
    ...
    if (ctx->n_free_cap == 0) {
        fprintf(stderr, "v4l2stateless: MPEG-2 no free capture buffer\n");
        ctx->n_free_out++;
        return VA_STATUS_ERROR_OPERATION_FAILED;
    }
    ...
    if (v4l2sl_queue_output(v4l2_fd, out_buf_idx,
                            ctx->output_buf_ptr[out_buf_idx]
                                ? ctx->output_buf_ptr[out_buf_idx]
                                : slice_datas[0],
                            (uint32_t)total, request_fd, timestamp) < 0) {
        fprintf(stderr, "v4l2stateless: failed to queue MPEG-2 output buffer\n");
        ctx->n_free_out++;
        ctx->n_free_cap++;
        return VA_STATUS_ERROR_OPERATION_FAILED;
    }
    ...
    if (v4l2sl_queue_capture(v4l2_fd, cap_buf_idx, request_fd) < 0) {
        fprintf(stderr, "v4l2stateless: failed to queue MPEG-2 capture buffer\n");
        ctx->n_free_cap++;
        return VA_STATUS_ERROR_OPERATION_FAILED;
    }

    if (v4l2sl_submit_request(request_fd) < 0) {
        fprintf(stderr, "v4l2stateless: failed to submit MPEG-2 request\n");
        return VA_STATUS_ERROR_OPERATION_FAILED;
    }

    {
        ...
        if (poll(&pfd, 1, 3000) <= 0) {
            fprintf(stderr, "v4l2stateless: MPEG-2 decode timed out\n");
            return VA_STATUS_ERROR_OPERATION_FAILED;
        }

        done_cap = v4l2sl_dequeue_buffer(v4l2_fd, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
        if (done_cap < 0) {
            fprintf(stderr, "v4l2stateless: MPEG-2 decode failed\n");
            return VA_STATUS_ERROR_OPERATION_FAILED;
        }
